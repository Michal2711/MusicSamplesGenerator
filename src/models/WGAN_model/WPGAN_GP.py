...
def gradient_penalty(disc, real, fake, device='cpu'):
    ...
    eps = torch.rand((b_size, 1, 1, 1)).repeat(1, c,h,w).to(device)

    interpolated_images = real * eps + fake * (1 - eps)

    mixed_scores, features_vector = disc(interpolated_images)
...
class WPGAN_GP(PGAN):
    ...
    def optimize_discriminator(self, real_images, b_size, feature_vectors, feature_vectors_fake):
        ...
        gp = gradient_penalty(self.discriminator, real_images, fake_images, device=self.device)
        d_loss = (
            -(torch.mean(real_output) - torch.mean(fake_output)) + self.lambda_gp*gp # original loss + lambda*gp
        )
    ...
    def optimize_generator(self, b_size, feature_vectors, feature_vectors_fake):
    ...
    def optimize_parameters(self, real_images, feature_vectors, feature_vectors_fake=None):
        ...
        for _ in range(self.n_critic):
            d_loss = self.optimize_discriminator(
                ...
                feature_vectors_fake=feature_vectors_fake
            )
